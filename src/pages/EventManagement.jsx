...
import {
    ...
    CheckCircle, Radio, Star, ChevronRight, Hash
} from 'lucide-react'
...
function Topbar() {
    return (
        <div className="topbar">
            <div>
                <div className="font-semibold" style={{ fontSize: '0.95rem' }}>Event Management</div>
                <div className="text-xs text-secondary">Manage and join active events</div>
            </div>
            <div className="flex items-center gap-12 ml-auto">
                <span className="text-xs text-muted">3 active events</span>
...
function statusStyle(s) {
    if (s === 'live') return { badge: 'badge-red', dot: 'crit', label: 'LIVE' }
    if (s === 'upcoming') return { badge: 'badge-amber', dot: 'warn', label: 'UPCOMING' }
    return { badge: 'badge badge-blue', dot: 'off', label: 'CLOSED' }
}
...
function CreateEventModal({ onClose }) {
    return (
        <div style={{
            ...
            <div className="card anim-scale" style={{ width: 480, maxHeight: '85vh', overflowY: 'auto', borderColor: 'var(--border-bright)' }}>
...
function JoinModal({ event, onClose }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
            <div className="card anim-scale" style={{ width: 420, borderColor: 'var(--border-bright)' }}>
            ...
            <div className="content-area">
                ...
                <div className="grid-4 mb-24">
                    {[
                        ...
                        { label: 'Total Participants', value: '332', icon: Users, color: 'var(--accent-blue)', bg: 'var(--accent-blue-dim)' },
                        { label: 'Open Alerts', value: '4', icon: Shield, color: 'var(--accent-amber)', bg: 'var(--accent-amber-dim)' },
                        { label: 'Completed Events', value: '18', icon: CheckCircle, color: 'var(--accent-green)', bg: 'var(--accent-green-dim)' },
                        ...
                        return (
                            <div key={ev.id} className={`card anim-fade-up delay-${i + 1}`}
                                style={{
                                    ...
                                    borderColor: ev.role === 'joined' ? 'rgba(59,130,246,0.4)' : ev.role === 'admin' ? 'rgba(139,92,246,0.3)' : 'var(--border)'
                                }}>
                                {/* Left: icon */}
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div className="flex items-center gap-10 mb-8 flex-wrap">
                                        ...
                                        <span className="text-xs text-muted" style={{ marginLeft: 'auto', fontFamily: 'JetBrains Mono, monospace' }}>{ev.id}</span>
                                    </div>
                                    <div className="text-sm text-secondary mb-12" style={{ lineHeight: 1.6 }}>{ev.desc}</div>

                                    <div className="flex items-center gap-20 flex-wrap mb-12">
                                        {[
                                    ...
                                    <div className="flex items-center gap-12 mb-8">
                                        ...
                                        {ev.alerts > 0 && (
                                            <span className="badge badge-red" style={{ marginLeft: 4 }}>{ev.alerts} alerts</span>
                                        )}
                                    </div>
